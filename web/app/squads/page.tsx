...
export default async function SquadsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      <h1 className="font-heading text-2xl font-bold text-slate-900">Trending Squads</h1>
      <p className="mt-1 text-sm text-slate-500">
        Every Squad below is currently gathering members. Join before the 24-hour window closes to lock in
        the group price.
      </p>

      {squads.length === 0 ? (
        <div className="mt-10 rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center text-sm text-slate-500">
          No Squads are forming right now — check back soon.
        </div>
      ) : (
